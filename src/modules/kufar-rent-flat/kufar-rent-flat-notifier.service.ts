...
import { buildListingCaption, buildSummary } from './kufar-rent-flat-format';

/** Tracks which listings were successfully delivered — service uses this to gate persistence. */
export interface KufarRentFlatNotifyResult {
  notifiedNew: Set<number>;
}

const emptyResult = (): KufarRentFlatNotifyResult => ({ notifiedNew: new Set() });

@Injectable()
export class KufarRentFlatNotifierService {
  private readonly logger = new Logger(KufarRentFlatNotifierService.name);
  ...
  constructor(
    ...
    this.sourceUrl = config.get<string>('kufarRentFlat.url') ?? '';
    if (!this.chatId) {
      this.logger.warn(
        'TELEGRAM_KUFAR_RENT_FLAT_CHAT_ID is not set — notifications disabled, nothing will be persisted',
      );
    }
  }
  ...
  async notifyError(message: string): Promise<void> {
    ...
    const ok = await this.telegram.sendMessage(
      this.chatId,
      `⚠️ Ошибка скрапинга travel.kufar.by:\n<code>${message}</code>`,
    );
    if (!ok) this.logger.warn('Failed to send kufar-rent-flat error notification');
  }

  private async sendListings(
    listings: KufarRentFlatListing[],
    header: string,
  ): Promise<Set<number>> {
