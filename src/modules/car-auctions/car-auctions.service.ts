...
/**
 * Business orchestration for the car auctions scrape cycle:
 ...
 */
@Injectable()
export class CarAuctionsService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(CarAuctionsService.name);
  ...
  constructor(
    ...
    private readonly notifier: CarAuctionsNotifierService,
  ) {}

  onModuleInit(): void {
    const cron = this.config.getOrThrow<string>('carAuctions.scrapeCron');
    const job = new CronJob(cron, () => {
      void this.runScheduled();
    });
    this.scheduler.addCronJob('car-auctions-scrape', job);
    job.start();
    this.logger.log(`Cron scheduled: ${cron}`);
  }

  onModuleDestroy(): void {
    this.scheduler.deleteCronJob('car-auctions-scrape');
  }
  ...
  private async scrape(): Promise<CarAuctionsResult> {
    const url = this.config.getOrThrow<string>('carAuctions.scrapeUrl');
    ...
    const result: CarAuctionsResult = {
